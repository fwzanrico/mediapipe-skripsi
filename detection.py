...
def write_computation_time(comp_time, filename='time_bs.csv'):
# Open the CSV file in append mode to keep adding frames' blendshapes
...
class FaceMeshDetector:
    def __init__(self):
        ...
        with open('blendshape_rf_model.p', 'rb') as f:
    ...
    def predict_expression(self, blendshapes):
        if blendshapes is None:
            return None, None
        model = self.classifier['model']
        prediction = model.predict([blendshapes])[0]
        confidence = np.max(model.predict_proba([blendshapes]))  # Use model. instead of self.classifier.
...
def main():
    ...
    while True:
        ...
        if landmarks is not None and blendshapes is not None:
            ...
            # Get prediction
            prediction, confidence = facemesh_detector.predict_expression(blendshapes)
